<script lang="ts">
    ...
    // Activation
    $: if(today.getDay() == 6 && (today.getTime() >= saturdays.getTime()) && (today.getTime() <= new Date(saturdays).setHours(14, 0, 0, 0))) {
        session = true;
    ...
    else if (today.getDay() >= 0 && (today.getTime() >= saturdays.getTime()) && (today.getTime() <= new Date(weekdays).setHours(19, 30, 0, 0))) {
        session = true
        let endTime = new Date();
        if(today.getHours() == 10) {
